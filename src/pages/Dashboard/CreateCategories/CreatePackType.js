...
export default function CreatePackType({ createPackTypeOpen, handlePackTypeOpen, handlePackTypeClose, setCreatePackTypeOpen }) {
    ...
    const createPackType= data => {
        ...
        fetch('https://soilight.herokuapp.com/inside/pack/type', {
            method: 'POST',
            headers: {
                "Content-type": "application/json",
                'Authorization': `Bearer ${userLogin?.user?.token}`
    ...
    return (
        <div>
            ...
            >
                <Box sx={style}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', cursor: 'pointer', fontSize: '20px' }} onClick={handlePackTypeClose}>
                    ...
                    <form onSubmit={handleSubmit(createPackType)}>
                        ...
                        <Box >
                            ...
                            <TextField fullWidth placeholder='Pack Type' size="small"    {...register("packType", { min: 0 })} required />
                        </Box>
